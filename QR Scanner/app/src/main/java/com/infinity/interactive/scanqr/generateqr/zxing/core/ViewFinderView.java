...
public class ViewFinderView extends View implements IViewFinder {
    ...
    public static final float DEFAULT_SQUARE_DIMENSION_RATIO = 5f / 7.5f;
...
//    private static final long ANIMATION_DELAY = 80L;
    ...
    private void init() {
        ...
        getmViewFinderHeight=CameraPreview.heightFinder;
        //set up laser paint
...
//        Toast.makeText(getContext(), framingRect.bottom+"", Toast.LENGTH_SHORT).show();
...
//        Log.d("laser",middle+"");
//        Log.d("laser",framingRect.height()+"");
//        Log.d("laser",framingRect.top+"");
//        Log.d("laser",framingRect.bottom+"");
//        Log.d("laser",middle+"");

        middle = middle + cntr;

        if ((cntr < framingRect.height()/2) && (!goingup)) {
            canvas.drawRect(mFramingRect.left + 2, middle - 1, mFramingRect.right - 1, middle + 2, mLaserPaint);
            cntr = cntr + 4;
        }

        if ((cntr >= framingRect.height()/2) && (!goingup)) goingup = true;
        ...
        if ((cntr <= -framingRect.height()/2) && (goingup)) goingup = false;
